...
//     //  1. VK_ERROR_OUT_OF_DATE_KHR: swap chain has become incompatible with the surface and cant be used for
//     rendering.
//     //  (Window resize)
//     //  2. VK_SUBOPTIMAL_KHR: swap chain can still be used to present to the surface, but the surface properties no
//     //  longer match
//     {
//         vk::Semaphore &acquireWait =
//             m_renderingContext.recordDependentSemaphores.get(this->imageAcquireSemaphores[frameIndexToBeDrawn]);
//         auto result =
//             this->device->getDevice().getVulkanDevice().acquireNextImageKHR(m_swapChain, UINT64_MAX, acquireWait);
